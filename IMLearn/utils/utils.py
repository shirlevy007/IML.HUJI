...
def split_train_test(X: pd.DataFrame, y: pd.Series, train_proportion: float = .75) \
    ...
    """
    ids = np.random.choice(X.index, size=int(train_proportion*len(X)), replace=False)
    train_X = X[X.index.isin(ids)]
    train_y = y[y.index.isin(ids)]
    test_X = X[~X.index.isin(ids)]
    test_y = y[~y.index.isin(ids)]
